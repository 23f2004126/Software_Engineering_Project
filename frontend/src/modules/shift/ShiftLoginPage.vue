...
const startShift = () => {
  ...
  // Use auth store to mark shift as started
  authStore.startShift()

  elapsedInterval = setInterval(() => {
    elapsedSeconds.value++
  }, 1000)

  // Redirect to dashboard after starting shift
  setTimeout(() => {
...
const confirmEndShift = () => {
  // Use auth store to mark shift as ended
  authStore.endShift()
      ...
      <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
        ...
        <div class="flex items-center gap-4 pb-6 border-b border-slate-100 mb-6">
          <div class="w-12 h-12 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-white font-semibold text-sm">
            {{ authStore.user?.name?.split(' ').map(n => n[0]).join('').toUpperCase() || 'EMP' }}
          </div>
          <div>
            <h2 class="text-xl font-semibold text-slate-900">{{ authStore.user?.name || 'Employee' }}</h2>
            <p class="text-sm text-slate-500">{{ authStore.user?.role || 'Staff' }}</p>
          </div>
        </div>
        ...
        <div class="bg-slate-50 rounded-2xl p-4 mb-6">
          <div class="flex items-center gap-2 mb-2">
            <div
              ...
              :class="authStore.shiftActive ? 'bg-emerald-400 animate-pulse' : 'bg-red-400'"
            ></div>
            <p :class="authStore.shiftActive ? 'text-emerald-600 font-semibold' : 'text-slate-600'">
              {{ authStore.shiftActive ? 'Shift Active' : 'Shift Not Started' }}
            </p>
          </div>
          <p v-if="authStore.shiftActive" class="font-mono text-sm text-slate-600 ml-4">
